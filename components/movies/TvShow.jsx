...
const TvShow = ({ data, to, screenName, header }) => {
    const { personUrl, Icon } = useMainContext();
    ...
    return (
        <View className='mt-4 pl-4 w-full'>
            <View>
                <Text className='text-2xl mb-2'>{header}</Text>
            </View>
            <ScrollView horizontal className=' pb-3'>
                {data.results.map((each) => (
                    <Link href={`/show/tv/${each.id}`} key={each.id} className='mr-4'>
                        <View className='w-24 h-24 bg-green-500 rounded-lg shadow-sm shadow-current' >
                            <Image
                                className='w-full h-full rounded-lg'
                                source={{ uri: `${personUrl}${each.poster_path}` }}
